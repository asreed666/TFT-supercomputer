...
void lightSetArrived(MQTT::MessageData &md) {
...
void tempSetArrived(MQTT::MessageData &md) {
  MQTT::Message &message = md.message;
  uint32_t len = md.message.payloadlen;
  //(&md.message.payload)[len-1] = 0;
  // int i = 0;
...
int main() {
  ...
  for (int j = 0; j < NUMLEDS_Y; j++) {
    for (int i = 0; i < NUMLEDS_X; i++) {
      array[i][j] = (RED_FADE ? 0x04ff0000 : 0) |
                    (GREEN_FADE ? 0x0200ff00 : 0) |
                    (BLUE_FADE ? 0x010000ff : 0);
  ...
  while (counter++ < 1000) {
    ...
    if ((array[ledx][ledy]) & 0x04000000) { // dim Blue
      ...
      if (array[ledx][ledy] & 0x08000000)
        shade = (shade + decrement) & 0xfc;
      else
        shade = (shade - decrement) & 0xfc;
    ...
    if ((array[ledx][ledy] & 0x00ffffff) == 0) {
      if ((array[ledx][ledy] & 0x08000000))
        array[ledx][ledy] = (array[ledx][ledy] ^ 0x08000000);
      else
        array[ledx][ledy] =
            (array[ledx][ledy] ^ 0x08000000) ^ ((rand() % 2) ? 0x04000000 : 0) ^
            ((rand() % 2) ? 0x02000000 : 0) ^ ((rand() % 2) ? 0x01000000 : 0);
      //            uint32_t currentColour
      //            =(RED_FADE?0x00ff0000:0)|(GREEN_FADE?0x0000ff00:0)|(BLUE_FADE?0x000000ff:0);
      //            GUI_SetColor(currentColour);
      //            GUI_FillCircle(LED_RADIUS+(ledx)*LED_DIAMETER,
      //            TITLE_OFFSET+LED_DIAMETER*(ledy), LED_RADIUS);
    } else {
  ...
  rc =
      client.subscribe((char *)LIGHT_THRESH_TOPIC, MQTT::QOS0, lightSetArrived);
  if (rc != 0)
  ...
  message.payloadlen = 80;
  rc = client.subscribe((char *)TEMP_THRESH_TOPIC, MQTT::QOS0, tempSetArrived);
  if (rc != 0)
    printf("Subscription Error %d\n", rc);
  else
    printf("Subscribed to %s\n", TEMP_THRESH_TOPIC);
  sprintf(buffer, "%d", tthresh);
  message.payload = (void *)buffer;
  message.payloadlen = strlen(buffer) + 1;
  rc = client.publish(TEMP_THRESH_TOPIC, message);
  if (rc == 0) {
    printf("publish temperature threshold  worked\n");
  } else {
    printf("publish temperature threshold failed %d\n", rc);
  }
  ...
  while (1) {
    ...
    if (rxCount != lastRxCount) {
      ...
      printf("tthresh is now = %d\n", tthresh);
      sprintf(buffer, "tthresh is now = %dC,", tthresh);
      GUI_DispStringAt(buffer, 0, 40);
